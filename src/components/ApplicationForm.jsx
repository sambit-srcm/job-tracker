...
export function ApplicationForm({ editTarget, onSubmit, onCancel, loading }) {
  ...
  // Pre-fill the form when an edit target is selected
  useEffect(() => {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-1.5">
          <Label htmlFor="date" className="text-gray-700">Date Applied *</Label>
          <Input
            id="date"
            type="date"
            value={form.date}
      ...
      <div className="flex gap-2 pt-1">
        ...
        >
          {isEditing ? <><Save className="mr-2 h-4 w-4" />Update</> : <><PlusCircle className="mr-2 h-4 w-4" />Add Application</>}
        </Button>
